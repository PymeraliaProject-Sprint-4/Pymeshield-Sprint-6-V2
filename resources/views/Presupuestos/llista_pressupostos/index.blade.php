            ...
            <tbody style="background-color: orange">
                @foreach ($data as $pressupost)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            {{ $pressupost->company_name }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $pressupost->created_at }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $pressupost->updated_at }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $pressupost->status }}
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('Asignar-Precios',  ['id' => $pressupost->id]) }}"
                                class="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded">Preu</a>
                        </td>
